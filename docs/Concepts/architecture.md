...
1. **Object Store:** Persists the physical variant content (the binaries). Konifer supports AWS S3 and any S3-compatible
   provider.
2. **Metadata Store:** Persists relational data, path hierarchies, tags, and labels. Currently, PostgreSQL is the
   supported engine.
...
For local development, unit testing, or trying out Konifer without infrastructure dependencies, you can enable in-memory
...
Konifer supports any S3 provider in addition to Amazon S3. This enables the usage of local storage options
like [MiniIO](https://www.min.io/)
